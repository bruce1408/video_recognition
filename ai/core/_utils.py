...
是否可以融合在compute_status之中省略此方法   不可以融合，本方法是对一些帧的预测结果的累加，所以说  offduty\onduty属于[0,帧数]之间的整数
'''
def compute_duty(duty_status):
    ...
    for k in camera_dic:
        ...
        for duty_info in camera_dic[k]:
            ...
            elif dic[k].get(duty_info['dutyid'], -1) != -1 and duty_info['prob_iou'] == 1:
...
def predict_box(frame, box_dic):
    ...
    for i in range(len(boxes_)):
        ...
        class_id = labels_[i]
        if class_id == 0:  ####之前是if class_id == 1: 需要将1改为0
...
def compute_status(true_box_list, predict_box_list, dutyid_list, ca_id):
    ...
    for t_box in true_box_list:
        ...
        for p_box in predict_box_list:
            ...
            count += iou
        if count >= 1:
            res_list.append((ca_id, {'dutyid': t_box[0], 'status': 'on_duty', 'prob_iou': 1}))  #不管几个人超过阈值iou都算作1
        else:
            res_list.append((ca_id, {'dutyid': t_box[0], 'status': 'off_duty', 'prob_iou': 0}))
    return res_list
...
def draw_bounding_box1(img, x, y, x_plus_w, y_plus_h, duty):
    ...
    cv2.putText(img, label, (x + 3, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 191, 0), 2)  # 黄色
